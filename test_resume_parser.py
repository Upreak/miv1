...
OPENROUTER_MODEL = "google/gemma-3-27b-it:free" # Example model to test OpenRouter connection
...
# Output settings
OUTPUT_FILE = "Resume_test_results.txt"

# ==============================================================================
# 2. THE PROMPT (Resume Parser)
# ==============================================================================
RESUME_PROMPT = """HTask: Extract all information explicitly mentioned in the resume(structured/unstructured text) and structure it according to the Candidate Portfolio format. Do NOT infer, guess, or generate missing details. If a field is not present, leave it blank.
...
# ==============================================================================
# 3. HELPER FUNCTION TO LOG RESULTS
# ==============================================================================
def log_result(provider, success, content):
    ...
    separator = "-" * 60
    # We will log the response in the file
    log_entry = (
        f"{separator}\n"
        f"Time: {timestamp}\n"
        f"Provider: {provider}\n"
        f"Status: {status}\n"
        f"Response (JSON extracted):\n{content}\n"
        f"{separator}\n"
    )
...
def test_gemini_direct():
    print("--> Testing Gemini (Google) Direct...")
    try:
        ...
        # Check if response was blocked or successful
        if response.text:
            log_result("Gemini Direct (Google)", True, response.text)
        else:
            log_result("Gemini Direct (Google)", False, "Response was empty (possibly blocked by safety filters)")
...
if __name__ == "__main__":
    print(f"Starting API Tests with Resume Parser Prompt.")
